...
async function reconcileBlenderWorkerJob(pieceId, jobId) {
    ...
    if (!response.ok) {
        const message = typeof body?.message === 'string' ? body.message : typeof body?.error === 'string' ? body.error : `Reconcile failed with status ${response.status}.`;
        throw new Error(message);
    }
    return body ?? {};
}
...
//# sourceMappingURL=app_d70929f4._.js.map