  ...
  const refreshData = useCallback(async () => {
    ...
    setPieces(
      (payload.pieces ?? []).map((piece) => {
        ...
        const pieceType = fitProfile?.pieceType;
        const garmentCategory: 'tops' | 'bottoms' | 'full-body' =
          pieceType === 'bottom' ? 'bottoms' : pieceType === 'full_body' ? 'full-body' : 'tops';
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <SectionBlock title="Wardrobe 2D Library" subtitle="Prepared pipeline status and mannequin-compatible fitting">
          <Tester2DWardrobePanel items={pieces} activePieceId={null} processingPieceId={null} onApply={applyPiece} />
        </SectionBlock>
      </div>
