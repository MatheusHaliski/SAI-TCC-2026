# RF29 — Desafios & Games do Guarda-Roupa

**Projeto:** FashionAI (SAI-TCC-2026)
**Status:** proposta de especificação (v0.1), para revisão do time
**Requisito novo:** RF29 (número liberado pela absorção do antigo RF29 em RF6, conforme [`02-rf-reestruturados-e-criterios-aceite.md`](../novo-projeto/02-rf-reestruturados-e-criterios-aceite.md) §1)
**Depende de:** RF5, RF6, RF8, RF19, RF25, RF26, RF27, RF28
...
|---|---|---|
| Destaques (RF27) | olha para trás | "O que o meu guarda-roupa é e como evoluiu?" |
| **Desafios (RF29)** | olha para frente | "O que vou fazer com ele nesta semana?" |
...
| `CHANEL_WEEK` | **Semana Chanel** | todo dia, aplicar "Tira uma coisa" (RF26) no look | 7 dias | ✅ | ✅ | — | — | Identidade |
...
- **Conquistas secretas** (RF27 §4.3 e [`04`](04-detalhes-de-engajamento.md)): se estivessem numa lista, deixariam de ser secretas.
...
| Evidência | Fonte |
|---|---|
| look montado | esquema criado (RF5/RF26) com as regras do §5 de [`02-inventory-score-calculo.md`](02-inventory-score-calculo.md): ≥ 2 peças e sobrevivência de 24 h |
...
| Desafio ativo | O que aparece no Meu Quarto (RF25) |
|---|---|
...
| Integração | Regra |
|---|---|
| Destaques (RF27) | sugere até 2 desafios para as dimensões mais fracas do score |
| Copilot (RF10) | conhece os desafios ativos. Se a regra do 10×10 estiver ativa, o Vista-me só sugere as 10 peças escolhidas |
| FAI Points (RF28) | recompensa na conclusão. Em Equipe, dividida **igualmente** entre quem terminou como membro, dentro dos limites diários |
...
|---|---|---|---|
| RF29.CA01 | usuário em Meu Guarda-Roupa | abre a sub-aba Desafios | vê o catálogo com os modos disponíveis por desafio, a dimensão do score que cada um melhora, a duração e a recompensa (§2.1) |
| RF29.CA02 | desafio que não aceita um modo (tabela §2) | o card é exibido | o modo não disponível não aparece como opção |
| RF29.CA03 | usuário escolhe o modo Solo | confirma os parâmetros | o desafio entra direto no estado `ativo` |
| RF29.CA04 | usuário escolhe Equipe ou Duelo | envia convites | só pode convidar quem segue (RF8) ou quem usou a Chave do Quarto. O desafio fica `aguardando` por até 48 h |
| RF29.CA05 | prazo de aceite encerrado sem o mínimo de participantes | o prazo vence | o desafio vai para `expirado`, sem penalidade para ninguém |
| RF29.CA06 | usuário com 3 desafios ativos | tenta iniciar ou aceitar outro | o sistema informa o limite e oferece encerrar um dos ativos |
| RF29.CA07 | desafio cooperativo com metas proporcionais | o progresso é calculado | o progresso da equipe é a média das frações das metas individuais. Nenhum valor absoluto de peças é exibido para os colegas |
| RF29.CA08 | duelo votado | um look é exibido para votação | marca, preço e autor ficam ocultos até o resultado ser publicado |
| RF29.CA09 | participante de equipe ou duelo | acessa o desafio | vê apenas os looks que cada membro publicou no desafio, **nunca** o guarda-roupa dos outros (**RNF6**) |
| RF29.CA10 | desafio ativo | o usuário cria um look ou registra um Look do Dia | o progresso é atualizado a partir do dado real (§4). Não existe botão "marcar como feito" |
| RF29.CA11 | participante sai de um desafio cooperativo | confirma a saída | a meta da equipe é recalculada com os membros restantes, e nenhum membro recebe notificação de culpa |
| RF29.CA12 | desafio concluído | o prazo acaba ou a meta é atingida | a recompensa é lançada no ledger de forma idempotente (RF28.CA01) e é gerado um card de resultado compartilhável |
| RF29.CA13 | desafio ativo com representação no quarto (§5) | o usuário abre Meu Quarto | o elemento visual do desafio aparece no ambiente e some quando o desafio termina |
| RF29.CA14 | desafio com regra de conjunto de peças (10×10, Temporada Cápsula) ativo | o usuário aciona o Vista-me ou o Copilot | as sugestões usam só as peças permitidas pela regra, e o Copilot avisa que está respeitando o desafio |
| RF29.CA15 | Destaques exibe o Inventory Score | a dimensão mais fraca está abaixo de 70 | a aba sugere até 2 desafios que melhoram essa dimensão |
| RF29.CA16 | bilhete com texto livre em equipe ou duelo | é enviado | passa pela moderação (RF15/RF16) e tem no máximo 80 caracteres. Não existe chat livre |
| RF29.CA17 | Desafio do Dia | o usuário conclui | o resultado é exibido como grade de emoji compartilhável, sem expor as peças nem o acervo |
...
| Fase | Escopo |
|---|---|
| **3a** (junto com a gamificação do RF28) | sub-aba, catálogo, modo **Solo** para 10×10, Segunda Chance, Sem Repetir e Semana Chanel. Progresso verificável. Representação no quarto |
