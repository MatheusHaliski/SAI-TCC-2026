...
export default function MyWardrobeView() {
  ...
  useEffect(() => {
    getServerSession().then((session) => {
      ...
      Promise.all([
        fetch(`/api/wardrobe-items/user/${userId}`).then((res) => res.json()),
        fetch(`/api/wardrobe-items/user/${userId}/analysis`).then((res) => res.json()),
      ]).then(([wardrobeItems, wardrobeAnalysis]) => {
        setItems(Array.isArray(wardrobeItems) ? wardrobeItems : []);
        setAnalysis(wardrobeAnalysis as Analysis);
      });
    });
  }, []);
