...
export default function DressTesterView() {
  ...
  const runTryOn = useCallback(async (piece: Tester2DWardrobeItem) => {
    ...
    if (!response.ok || payload.status !== 'completed' || !payload.resultImageUrl) {
      setStageError(payload.error ?? 'Could not fit garment right now.');
      return;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(280px,360px)_minmax(0,1fr)_380px]">
        ...
        <SectionBlock title="Editing Stage" subtitle="Automatic fitted mannequin render">
          <Tester2DStage mannequinImageUrl={mannequin.baseImageUrl} displayImageUrl={stageImageUrl || mannequin.baseImageUrl} loading={Boolean(processingPieceId)} error={stageError} onRetry={() => { const piece = pieces.find((p) => p.pieceId === selectedPieceId); if (piece) void runTryOn(piece); }} />
        </SectionBlock>
