...
export default function CreateMySchemeView() {
  ...
  const saveScheme = async (creation_mode: 'manual' | 'ai') => {
    ...
    const response = await fetch('/api/schemes', {
      ...
      body: JSON.stringify({ user_id: userId, title, style, occasion, visibility, creation_mode, items: schemeItems }),
    });

    if (!response.ok) {
      const payload = (await response.json().catch(() => ({ error: 'Unable to save scheme' }))) as { error?: string };
      alert(payload.error ?? 'Unable to save scheme');
      return;
    }
