  ...
  return (
    ...
    >
      {Array.from({ length: SKELETON_COUNT }).map((_, i) => (
        <div
          key={i}
          className={`runway-skeleton ${
            mode === 'grid'
              ? 'aspect-square'
              : mode === 'runway'
                ? 'h-screen'
                : 'h-[400px] break-inside-avoid mb-3'
          }`}
        />
      ))}
    </div>
...
const MODE_ICONS: Record<FeedMode, string> = {
  magazine: '⊞',
  runway: '⊡',
  grid: '⊟',
};
...
export default function RunwayFeedView({
  viewerId,
  viewerName,
  viewerPhotoUrl,
}: RunwayFeedViewProps) {
  ...
  const loadSchemes = useCallback(
    async (pageIndex: number, currentFilter: FeedFilter) => {
  ...
  const GRID_CLASS: Record<FeedMode,string> = {
    ...
    runway:
      'flex flex-col snap-y snap-mandatory overflow-y-scroll h-screen',
      ...
      ">
        ...
        ">
          ...
          ">
            ...
            {(Object.keys(MODE_ICONS) as FeedMode[])
              .map((m)=>(
        ...
        <div className="flex gap-2">
          ...
          {(Object.keys(FILTER_LABELS) as FeedFilter[])
          .map((f)=>(
...

        {
          ...
          ">
            ...
            <p className="
              ...
              text-white/20
            ">
              Seja o primeiro a compartilhar um look!
            </p>
