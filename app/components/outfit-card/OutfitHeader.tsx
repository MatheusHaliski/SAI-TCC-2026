  ...
  return (
    <header className={`space-y-2 rounded-2xl border border-white/20 bg-black/25 shadow-[0_8px_24px_rgba(15,23,42,0.22)] backdrop-blur-md ${compact ? 'p-3' : 'p-4'}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {creatorName ? <span className="text-xs text-white/70">por @{creatorName}</span> : null}
          </div>
          {brandBadges.length ? (
            <div className="flex flex-wrap items-center gap-2">
      ...
      <p className="text-sm font-medium text-cyan-100">
        {outfitStyleLine}
        {occasion ? <span className="ml-2 text-white/55">· {occasion}</span> : null}
      </p>

      {/* Community stats row — stars · owners · likes */}
        ...
        <div className="flex flex-wrap items-center gap-3">
          {ratingStars !== undefined ? (
            <span className="inline-flex items-center gap-1">
              ...
              <span className="font-mono text-[10px] text-white/70">{ratingStars.toFixed(1)}</span>
            </span>
          ) : null}
