...
} from '@/app/lib/fashion-ai/background-studio/procedural';
import {
...
/* ── helpers ── */
const STORAGE_KEY = 'sai_page_background_config';
...
function buildShapeOverlaySvg(shape: PageBackgroundShape, color: string): string {
  ...
  return `data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='1200' height='800'><defs>${inner.includes('radialGradient') ? '' : ''}</defs>${inner}</svg>`)}`;
}
...
export default function BackgroundStudioPanel({ onSaved }: BackgroundStudioPanelProps) {
  ...
  /* build current CSS background from active tab */
  const buildCurrentBackground = useCallback((): string => {
    if (tab === 'cor') return solidColor;
    if (tab === 'gradiente') return buildGradientCss(gradStops, gradType, gradAngle);
    if (tab === 'geometria') {
      const base = `linear-gradient(135deg, #0f172a 0%, #1e0a3c 100%)`;
      const svg = buildShapeOverlaySvg(shape, shapeColor);
      return svg ? `${svg ? `url("${svg}"), ` : ''}${base}` : base;
    }
    return ''; // presets handled separately
  }, [tab, solidColor, gradStops, gradType, gradAngle, shape, shapeColor]);

  /* live preview: apply on every state change */
  useEffect(() => {
    if (tab === 'presets') return;
    const bg = buildCurrentBackground();
    if (bg) {
      document.documentElement.style.setProperty('--home-shell-bg', bg);
      document.documentElement.style.setProperty('--background', tab === 'cor' ? solidColor : '');
    }
  }, [tab, buildCurrentBackground, solidColor]);

  const applyAndSave = () => {
    let gradient = buildCurrentBackground();
    if (tab === 'presets' || !gradient) gradient = buildCurrentBackground();
    ...
    if (typeof window !== 'undefined') {
      ...
      if (activePresetId) window.localStorage.setItem(ACTIVE_BG_KEY, activePresetId);
    }
    setSaveMsg('✓ Fundo salvo!');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      {tab === 'gradiente' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {/* Type */}
          <div>
            {sectionTitle('Tipo de gradiente')}
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              {(['linear', 'radial'] as const).map(t => (
                <button key={t} type="button" onClick={() => setGradType(t)} style={{ flex: 1, padding: '0.5rem', borderRadius: '0.5rem', border: `1px solid ${gradType === t ? 'var(--primary)' : 'var(--border)'}`, background: gradType === t ? 'rgba(124,58,237,0.12)' : 'var(--accent)', color: gradType === t ? 'var(--primary)' : 'var(--foreground)', fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer', fontFamily: 'inherit' }}>
          ...
          {/* Stops */}
          <div>
            {sectionTitle('Cores do gradiente')}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              {gradStops.length < 5 && (
                <button type="button"
                  onClick={() => setGradStops([...gradStops, { color: '#ffffff', position: 100 }])}
      ...
      {tab === 'cor' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
            {[
              '#0d0b18','#0f172a','#111111','#1a1a2e','#0a0f1e',
              '#f8f7ff','#ffffff','#f3f4f6','#fdf4ff','#fff0f6',
              '#7c3aed','#4c1d95','#db2777','#0ea5e9','#10b981',
            ].map(c => (
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        {saveMsg && <span style={{ fontSize: '0.875rem', color: '#10b981', fontWeight: 600 }}>{saveMsg}</span>}
      </div>
    </div>
  );
}
